...
### 1.5a CUDA DNN fast gate — no model files needed (run first)
...
**Pass (home PC / correct build):**
```
OpenCV:     4.11.0
From:       /usr/local/lib/python3.10/dist-packages/cv2/__init__.py
CUDA DNN:   COMPILED IN
Backends:   [(5, 5), (5, 6)]    ← (CUDA FP32, CUDA FP16)
```

**Fail (lab asset — expected):**
```
OpenCV:     4.8.0               ← system apt, or wrong PYTHONPATH
From:       /usr/lib/...        ← must be /usr/local/lib/python3.10/dist-packages/
CUDA DNN:   NOT COMPILED — rebuild required
Backends:   [(0, 0)]            ← CPU only
```

**If NOT COMPILED:** Stop here — go straight to rebuild with
`install_opencv4.13.0_Jetpack6.2.1.sh`. No point running the full probe.  
**If COMPILED IN:** Continue to Step 1.5b.

---

### 1.5b CUDA inference probe — full end-to-end test (run after 1.5a passes)

Catches subtler failures: `setInputParams()` ordering issues, CUDA kernel launch errors,
or a correct compile but broken runtime — none of which show up in `getAvailableBackends()`.

```bash
# Pre-check: model files must exist or probe gives a false file-not-found failure
ls ~/CV/TRCv3/v20/model_data/frozen_inference_graph.pb \
   ~/CV/TRCv3/v20/model_data/ssd_mobilenet_v3_large_coco_2020_01_14.pbtxt \
   ~/CV/TRCv3/v20/model_data/coco.names
# All three must exist before running the probe below
...
**All OK:** Baseline confirmed — COCO inference running on GPU.  
**CUDA FP16/FP32 FAILED, CPU OK:** Compiled in but not working — check cuDNN headers,
`LD_LIBRARY_PATH`, and OPENCV_CUDA_BUILD.md §5 troubleshooting.  
**File not found error:** model_data/ missing on this machine — copy from home PC or
source. Not a CUDA issue.
