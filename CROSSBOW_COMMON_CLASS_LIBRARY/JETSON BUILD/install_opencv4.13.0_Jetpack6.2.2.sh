...
echo ""
read -r -p "Review pre-flight results above. Press Enter to continue (or type 'no' to abort): " CONT
[[ "$CONT" == "no" ]] && abort "Aborted by user."
echo ""
info "--- Existing OpenCV ---"
for (( ; ; )); do
    read -r -p "Remove system/apt OpenCV packages before building? Press Enter for yes, type 'no' to skip: " RM_OLD
    if [[ "$RM_OLD" == "no" ]]; then
...
check_cache "WITH_OPENGL"        "OFF"
check_cache "CUDA_FAST_MATH"     "ON"
check_cache "ENABLE_FAST_MATH"   "ON"
...
echo ""
echo "--- cmake gate complete ---"
echo ""
read -r -p "Review cmake flags above then press Enter to start build (or type 'no' to abort): " CONT
[[ "$CONT" == "no" ]] && abort "Aborted by user."
echo ""
